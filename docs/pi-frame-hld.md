...
| **Vertical slider widget** | pygame has no native vertical slider widget. No system widget library is in scope (all UI is custom pygame). | A custom `VerticalSlider` widget is required (see Section 5.2). It renders a thin vertical track with a circular draggable thumb. Internal value range 0–100 maps to a sysfs write range of 0–255. |
