...
echo "  ssh $PI 'kill -9 \$(cat /run/user/1000/slideshow.pid 2>/dev/null) 2>/dev/null; sleep 1; XDG_RUNTIME_DIR=/run/user/1000 WAYLAND_DISPLAY=wayland-0 ${REMOTE_DIR}/.venv/bin/slideshow > /tmp/slideshow.log 2>&1 &'"
echo ""
echo "Debug commands:"
echo "  ssh $PI 'cat /tmp/slideshow.log'           # slideshow logs"
echo "  ssh $PI 'cat /run/user/1000/slideshow.pid'   # slideshow PID"
