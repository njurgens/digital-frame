# T16: Ensure all existing tests pass and `eng/check.sh` is clean
...
svc2.stop()
del os.environ['PIFRAME__SYNC__PROVIDER']
...
bash eng/check.sh
git add -A
git commit -m "T16: Ensure all existing tests pass and `eng/check.sh` is clean"
```
